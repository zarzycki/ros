...
fig, axs = plt.subplots(npanels, 1, figsize=(10, 7), gridspec_kw={'hspace': 0})
...
axs[0].axhline(y=pthresh, color='grey', linestyle='--', label=f'$t_{{PRECIP}}$ = {pthresh:.2f}')
...
axs[1].axhline(y=wthresh, color='grey', linestyle='--', label=f'$t_{{ROF}}$ = {wthresh:.2f}')
...
axs[2].axhline(y=sthresh, color='grey', linestyle='--', label=f'$t_{{dSWE}}$ = {sthresh:.2f}')
axs[2].set_xlabel('Date')
axs[2].set_ylabel('dSWE')
axs[2].legend()
if plot_fraction:
    axs[2].set_xticks([])
